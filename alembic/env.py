...
from config import config as nlu_config
...
conf.set_main_option(
    ...
    'postgres://{user}:{password}@{host}:{port}/{db}'.format(
        user=nlu_config.PG_USER,
        password=nlu_config.PG_PASSWORD,
        host=nlu_config.PG_HOST,
        port=nlu_config.PG_PORT,
        db=nlu_config.PG_APP_DB
    )
)
